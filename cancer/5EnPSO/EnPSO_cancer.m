%% Arijit Nandi , National Institute of Technology Durgapur , jit.ari172@gmail.com
%% -----------------------------------------------------------------------------
clc
clear all
close all
...
load Training_cancer.mat
load Testing_cancer.mat
x=Training;
...
%////////Social component///////////
gBestScore=inf; %initialize gBestScore for min
gBest=zeros(1,Dim);
%///////////////////////////////////

ConvergenceCurve=zeros(1,Max_iteration); %Convergence vector

%Initialization of velocity & Position
for i=1:size(pos,1) % For each Particle
    for j=1:size(pos,2) % For each dimension
        pos(i,j)=lb+sobolnumber(0,1,1)*(ub-lb); % initial position initialization
        vel(i,j)=vmin+sobolnumber(0,1,1)*(vmax-vmin); %initial velocity initialization
    end
end
...
for Iteration=1:Max_iteration
    ...
    w= wMin+tanh(Iteration*((wMax-wMin)/Max_iteration));
    %         w1(Iteration) = w;
    %         ta(Iteration) = tanh(Iteration*((wMax-wMin)/Max_iteration));
...
classerr=(1-(Rrate/TestingingNO))*100;
...
%perf = crossentropy(Trgt_op,NN_op);
...
disp(['sensitivity:' num2str(Cval(1,2)*100) '%']);
disp(['specificity:' num2str(Cval(1,3)*100) '%']);
disp(['precision' num2str(Cval(1,4)*100) '%']);
disp(['recall:'  num2str(Cval(1,5)*100) '%']);
disp(['f_measure:' num2str(Cval(1,6)*100) '%' ]) ;
disp(['gmean:' num2str(Cval(1,7))  ]);
disp(['AUC:' num2str(Cval(1,8))  ]);
...
if ClassificationRate >96
    ...
    %% Saving Files
    filename=[strn 'FNNPSO_cancer POP ' num2str(noP) ' C rate' num2str(ClassificationRate) '.mat'];
    filename1=[strn 'FNNPSO_cancer POP ' num2str(noP) ' C rate' num2str(ClassificationRate) ' CPlot.fig'];
    filename2=[strn 'FNNPSO_cancer POP ' num2str(noP) ' C rate' num2str(ClassificationRate) ' Rplot.fig'];
    filename3=[strn 'FNNPSO_cancer POP ' num2str(noP) ' C rate' num2str(ClassificationRate) ' ROCplot.fig'];
    filename4=[strn 'FNNPSO_cancer POP ' num2str(noP) ' C rate' num2str(ClassificationRate) ' ConfusionMat plot.fig'];
    save(filename);%Saving workspace
    save(figure(1),filename1)
    save(figure(2),filename2)
    save(figure(3),filename3)
    save(figure(4),filename4)
